...
```
**Figure 1.** Annual juvenile snowshoe hare trap count in Bonanza Creek Experimental Forest between 1998-2012. This total reflects the number of juvenile hares trapped each year during this study. Data: Kielland et al. 2017
...
ggplot(data = snowshoe_weight, aes(sex,weight)) +
  geom_jitter(aes(color = sex), size = 1.8, width = .35) +
  facet_wrap(~site) +
...
plot(snowshoe_lm) #check homoscedasticity 
```
On average, male juvenile snowshoe hares have a higher body mass than female juveniles (`r snowshoe_stats$mean[2]` $\pm$ `r snowshoe_stats$std_dev[2]` and `r snowshoe_stats$mean[1]` $\pm$ `r snowshoe_stats$std_dev[1]` g, respectively). The absolute difference in means is `r snowshoe_stats$mean[2] - snowshoe_stats$mean[1]` g (a `r diff_mean`% difference), however the difference in means is significant (Welch's two-sample t-test: t(`r round(snow_t$parameter,2)`) = `r round(snow_t$statistic,2)`, p < 0.05), and the effect size is large (Cohen's *d* = `r round(snow_cohen$estimate,2)`).
...
```
The relationship between juvenile hair hind foot length and body mass appears relatively linear. Simple linear regression revealed that hind foot length significantly predicts penguin body mass (p < 0.05, R^2^ = `r round(snowshoe_lm_glance$r.squared,2)`) with an average slope of $\beta$ = `r round(snowshoe_lm_tidy$estimate[2], 2)` g mm^-1^ (i.e., for each one millimeter increase in hind foot length we expect an average increase in body mass of `r round(snowshoe_lm_tidy$estimate[2], 2)` g). There is a moderate correlation between hind foot length and body mass (Pearson's *r* = `r round(showshoe_cor_tidy$estimate,2)`, p < 0.05). Diagnostic plots (not included) reveal normally distributed and heteroscedastic residuals.
...
ggplot(data = snowshoe, aes(x = hindft , y = weight)) +
  geom_point(color = "#800080", size = 1) +
  labs(x = "Hind Foot Length (mm)",
       y = "Body Mass (g)") +
  theme_hc() +
  theme(legend.position = "none") #dopping legend
```
...
- Juvenile snowshoe hare populations fluctuate year to year and may experience large spikes eery 2-4 years, however more data is needed to confirm this assertion. 
